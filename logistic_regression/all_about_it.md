...
- first difference is the formula of the model which is: y_pred=𝜎(z)=1/1+e^-z where z=m*x+c
   - y_pred is the predicted value and the z is sigmoid function 𝜎(𝑧) which squashes the output between 0 and 1.
2) we calculate the loss using the loss function, which is a common step done. from many of the loss functions here we use the 
